      ...
      const updated = Array.isArray(list)
        ? list.map((a) => {
            ...
            return {
              ...
              status: nextStatus,
              previousStatus,
              resolvedAt: isResolved ? "" : new Date().toISOString(),
              activityLog: [
                ...(a.activityLog || []),
                {
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs text-muted-foreground mb-3">
                    ...
                    {activity.status === "Resolved" && activity.resolvedAt && (
                      <div className="flex items-center space-x-1">
                        <Clock className="h-3 w-3" />
                        <span>Resolved: {new Date(activity.resolvedAt).toLocaleString()}</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="space-y-3">
                    ...
                    {selectedTicketActivity.status === "Resolved" && selectedTicketActivity.resolvedAt && (
                      <div>
                        <p className="text-sm font-medium text-gray-700">Resolved At</p>
                        <p className="text-sm text-gray-600">
                          {new Date(selectedTicketActivity.resolvedAt).toLocaleString()}
                        </p>
                      </div>
                    )}
                  </div>
      ...
      >
        <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid gap-4 py-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="ticket-date">Registration Date</Label>
                <Input
                  id="ticket-date"
                  type="date"
                  value={newTicket.date}
                  onChange={(e) =>
                    setNewTicket((v) => ({ ...v, date: e.target.value }))
                  }
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="ticket-time">Registration Time</Label>
                <Input
                  id="ticket-time"
                  type="time"
                  value={newTicket.time}
                  onChange={(e) =>
                    setNewTicket((v) => ({ ...v, time: e.target.value }))
                  }
                />
              </div>
            </div>
                ...
                try {
                  ...
                  if (isEditingTicket && editingActivityId != null) {
                      ...
                      ? list.map((a) =>
                          a.id === editingActivityId
                            ? {
                                ...
                                date: newTicket.date || a.date || "",
                                time: newTicket.time || a.time || "",
                              }
                            : a,
                        )
                      : [];
                    ...
                    const activity = {
                      ...
                      linkedClient: newTicket.client,
                      unitType: "Government",
                      date: newTicket.date || now.toISOString().split("T")[0],
                      time: newTicket.time || now.toTimeString().slice(0, 5),
                      responsible: newTicket.assignees,
                      status: "To Do",
                      deadline: newTicket.dueDate || "",
                      reminderDate: "",
